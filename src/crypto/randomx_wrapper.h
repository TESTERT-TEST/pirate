...
// Copyright (c) 2026 The Hush developers
// Copyright (c) 2026 Miner113 developers
...
/**
 * RandomX wrapper for Hush
 *
 * RandomX is a proof-of-work algorithm optimized for general-purpose CPUs.
 * This wrapper provides a simple interface for using RandomX in Hush.
 */

// RandomX epoch configuration defaults
// These values match DragonX/Hush consensus defaults.
// Previously runtime-configurable via -ac_randomx_interval and -ac_randomx_lag.
static const uint64_t RANDOMX_DEFAULT_EPOCH_BLOCKS = 1024;  // Default: 1024 blocks per epoch
static const uint64_t RANDOMX_DEFAULT_EPOCH_LAG = 64;       // Default: 64 block lag for seed activation

/**
 * Calculate the seed height for a given block height.
 * The seed changes every 1024 blocks with a 64-block lag.
 * First epoch transition occurs at block 1088 (1024 + 64).
 *
